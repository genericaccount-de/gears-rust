...
**toolkit-auth PKCE `S256` code-challenge — routed through the FIPS provider, no allow-list entry.** The PKCE (RFC 7636) `S256` code-challenge `BASE64URL(SHA256(verifier))` in `libs/toolkit-auth/src/oauth2/authcode.rs` needs a real SHA-256. Rather than importing RustCrypto `sha2` (which would require a DE0708 allow-list entry **and** still trip the `cargo-deny` FIPS dependency gate as a new direct dep), it computes the digest via `aws-lc-rs` — the same FIPS-validated provider the platform installs at bootstrap — mirroring the ledger idempotency fingerprint. No direct `sha2` dependency is introduced, so both gates stay green.
