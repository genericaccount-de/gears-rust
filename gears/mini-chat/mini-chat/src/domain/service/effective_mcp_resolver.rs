...
/// MCP server and original tool. Consumed by the agentic loop to
...
pub struct McpToolRoute {
    ...
    #[allow(dead_code)] // carried for diagnostics; not read on the dispatch path
    ...
    #[allow(dead_code)] // resolved for completeness; dispatch uses the pool's timeout
...
// `resolve` is the production entry point; `get`/`len`/`is_empty` are used by tests.
#[allow(dead_code)]
...
impl EffectiveResolution {
    /// Whether any MCP tool was resolved. Test-only convenience.
    #[must_use]
    #[allow(dead_code)]
    pub fn is_empty(&self) -> bool {
        self.tools.is_empty()
    ...
    for EffectiveMcpResolver<MSR, MTR>
{
    ...
    async fn resolve(
        ...
        for g in &cached.gated {
            if !self.is_connected(ctx, subject, g.upstream_id).await {
