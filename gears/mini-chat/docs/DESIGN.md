...
| `OAuth2AuthorizationCode { scopes }` | `oauth2_auth_code` (`gts.cf.core.oagw.auth_plugin.v1~cf.core.oagw.oauth2_auth_code.v1`) | `token_ref` (deterministic per-user token store key `mcp_oauth_{server_id}`), `resource` (the MCP server URL, used as the protected-resource identifier), `scopes` — no secret refs; OAGW owns dynamic client registration, PKCE, and the per-user token store |
...
Mini-chat never sees the authorization code exchange, refresh tokens, or client secrets — it only relays `state`/`code` and reads a boolean status. Gateway failures surface as the canonical `service_unavailable` error (`503 Service Unavailable`, `Retry-After: 5`); OAGW's own canonical error detail is not forwarded to the client.
