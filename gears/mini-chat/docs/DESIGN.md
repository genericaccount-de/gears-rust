...
1. **Begin** (`POST /v1/mcp-servers/{id}/connection:authorize`, action `manage_mcp_connection`) — validates the server is `oauth2_auth_code` and has a provisioned `oagw_upstream_id`, and calls OAGW `begin_oauth_authorization(upstream_id, return_to, client_name)` with the caller-supplied `return_to` (where the browser should land after enrollment). Neither `scopes` nor the OAuth `redirect_uri` are caller-supplied: OAGW sources `scopes` from the upstream's stored `auth_config` and uses its own deployment-configured callback URL as the `redirect_uri`, and it validates `return_to` against its configured allowlist. Returns `{ authorization_url, state }`. The client opens `authorization_url` in a browser/popup; when enrollment completes, OAGW's callback exchanges the code and redirects the browser to `return_to` (with `oauth=success|error` appended).
...
- `BeginMcpConnectionReq` — `{ return_to: String }` (the allowlisted URL the browser lands on after enrollment; the OAuth `redirect_uri` is OAGW's own deployment-configured callback, never client-supplied); `BeginMcpConnectionResp` — `{ authorization_url: String, state: String }`
