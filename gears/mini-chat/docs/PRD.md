...
| `OAuth2AuthorizationCode { scopes }` | `oauth2_auth_code` | `token_ref` (deterministic per-user token store key `mcp_oauth_{server_id}`), `resource` (the MCP server URL as the protected-resource identifier), `scopes` — no secret refs; OAGW owns dynamic client registration, PKCE, and the per-user token store |
