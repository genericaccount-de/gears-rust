...
impl OAuthEnrollmentService for OAuthEnrollmentServiceImpl {
    ...
    async fn complete(
        ...
        warn!(
            ...
            "MCP-INVESTIGATE: OAGW complete storing per-user token record"
        );
        self.credstore
    ...
    async fn revoke(&self, ctx: &SecurityContext, upstream_id: Uuid) -> Result<(), DomainError> {
        ...
        warn!(
            ...
            %upstream_id,
            token_ref = ?token_ref,
            "MCP-INVESTIGATE: OAGW revoke deleting per-user token record"
        );
        self.credstore
    ...
    async fn status(
        ...
        let Some(resp) = resp else {
            warn!(
                ...
                %upstream_id,
                token_ref = ?token_ref,
                "MCP-INVESTIGATE: OAGW status -> NOT connected (no token record in credstore)"
            );
        ...
        warn!(
            ...
            expires_at_unix = record.expires_at_unix,
            now_unix = now_unix(),
            expired = record.expires_at_unix <= now_unix(),
            "MCP-INVESTIGATE: OAGW status -> connected (token record present)"
        );
