...
impl Service {
    ...
    /// config-only.
    pub fn put(&self, ctx: &SecurityContext, key: &SecretRef, value: SecretValue, sharing: SharingMode) {
        let tenant_id = TenantId(ctx.subject_tenant_id());
        ...
        match sharing {
            SharingMode::Private => {
                store.private_secrets.insert(
                    ...
                    SecretEntry { value, sharing, owner_id, owner_tenant_id: tenant_id },
                );
            }
            SharingMode::Tenant => {
    ...
    /// Delete the secret resolved for the caller's context, following the same
    /// **Private → Tenant → Shared → Global** precedence as [`Service::get`].
    ///
    ...
    pub fn delete(&self, ctx: &SecurityContext, key: &SecretRef) {
        ...
        if store.tenant_secrets.remove(&(tenant_id, key.clone())).is_some() {
            return;
        }
        if store.shared_secrets.remove(&(tenant_id, key.clone())).is_some() {
            return;
        }
        store.global_secrets.remove(key);
    }
}
