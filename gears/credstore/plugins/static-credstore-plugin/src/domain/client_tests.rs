...
async fn put_then_get_returns_stored_private_secret() {
    ...
    plugin
        .put(&caller, &key, SecretValue::from("access-token"), SharingMode::Private)
        .await
        .unwrap();
...
async fn delete_is_idempotent_for_missing_key() {
    ...
    plugin.delete(&ctx(tenant_a(), owner_a()), &key).await.unwrap();
}
...
async fn private_put_isolated_across_owners() {
    ...
    assert!(plugin.get(&ctx(tenant_a(), owner_b()), &key).await.unwrap().is_none());
    // The owner can.
    assert_eq!(
        plugin
            .get(&ctx(tenant_a(), owner_a()), &key)
...
async fn private_put_isolated_across_tenants() {
    ...
    assert!(plugin.get(&ctx(tenant_b(), owner_a()), &key).await.unwrap().is_none());
}
